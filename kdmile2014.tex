...
No Algoritmo \ref{alg:modificado} são exibidas (marcadas em verde) as
modificações em relação à proposta original, realizadas neste
trabalho.
...
\begin{algorithmic}[1]                 % enter the algorithmic environment
    ...
    \STATE\label{alg:mod:oenil}\grifar{$o_e \leftarrow nil$}

    \IF{ocorreu um evento saliente $e$}

        \IF{macro-ação para $e$, $o_e$, não existe em $O$}

            \STATE\label{alg:mod:oeo}\grifar{$o_e \leftarrow $[representação da macro-ação associada a $e$]}
    ...
    \FOR{cada macro-ação $o \neq $\grifar{$o_e$} em $O$}
        ...
        \ENDIF
        \IF{\grifar{$a_t$ é a ação} \tr{greedy} \grifar{para $o$ no estado $s_t$}}
            \STATE\COMMENT{Atualiza o modelo probabilístico de transição da macro-ação}

            \STATE $P(x|s_t) \xleftarrow{\alpha} [\gamma(1 -
            \beta^o(s_{t+1}))P^o(x|s_{t+1}) +
            \gamma\beta^o(s_{t+1})\delta_{s_{t+1}x}]$

            \STATE\COMMENT{Atualiza modelo de recompensa da
              macro-ação} \STATE $R^o(s_t) \xleftarrow{\alpha}
            [$\grifar{$r^e_{t+1}$}$ + \gamma(1 -
            \beta^o(s_{t+1}))R^o(s_{t+1})]$
        \ENDIF
    \ENDFOR

    \GIMMESPACE
    \STATE\COMMENT{Atualização da \tr{função valor estado-ação} do Q-Learning}
    \STATE $Q_B(s_t,\ a_t) \xleftarrow{\alpha} $\grifar{$r^e_{t+1}$}$ + $\grifar{$r^i_{t+1}$}$ +
    \gamma\max_{a \in A \cup O}{Q_B(s_{t+1},\ a)}$
    ...
    \FOR{cada macro-ação $o$ em $O$ tal que $s_t \in I^o$}
        \STATE $v \leftarrow$ \tv
        \STATE $Q^o(s_t,\ a_t)$\aalpha$\ $\grifar{$r^e_{t+1}$}$\ +\ \gamma\{$\termoA$\ +\ $\termoB$\}$
        \FOR{cada macro-ação $o'$ em $O$ tal que $s_t \in I^{o'}$ e $o \neq o'$}
            \STATE $Q^o(s_t,\ o')$\aalpha $R^{o'}(s_t)\ + \sum_{x \in
              S}{P^{o'}(x|s_t)}\{$\termoC$\ +\ $\termoD$\}$
        \ENDFOR
    \ENDFOR

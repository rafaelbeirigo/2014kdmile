...
denominada \emph{ótima}, que pode ser obtida através de técnicas de
Aprendizado por Reforço (AR) \cite{rlczaba}, nas quais o agente
experimenta ações em estados ainda não visitados através de
exploração, guiando sua trajetória através do conhecimento já
adquirido, em um compromisso entre a descoberta de informação sobre o
ambiente e a utilização do conhecimento adquirido.
...
parciais \cite{option1999}, definidas por $<\mathcal{I}, \pi, \beta>$,
em que $\mathcal{I} \subseteq S$ indica os estados em que a política
parcial pode ser iniciada, $\beta: S \rightarrow [0,1]$ a
probabilidade de terminar a política parcial em um estado $s \in S$ e
$\pi:S \rightarrow \mathcal{O}$, associa ações primitivas ou políticas
parciais a serem executas, sendo que $\mathcal{O}$ corresponde ao
conjunto de todas as ações primitivas e políticas parciais.
...
resolvê-lo.  Neste trabalho propõe-se a adição de duas funcionalidades
a esse algoritmo, a saber: utilização de uma \emph{pilha} de políticas
parciais e exploração \emph{interna às políticas parciais}.  No
momento da obtenção da próxima ação, feita de forma aleatória ou
gulosa, caso a próxima ação selecionada seja uma política parcial
$p_1$, esta é adicionada ao topo da pilha \pilha.  Dessa forma, um
agente que inicia com uma pilha vazia $\pilhaS=[\ ]$ e obtém como
próxima ação a política parcial \peum\ resultará com
$\pilhaS=[\peumS]$. Caso a aplicação de \peum\ sugira a execução de
uma nova política parcial $p_2$, essa será então empilhada, resultando
em $\pilhaS=[\peumS, p_2]$ e assim sucessivamente para novas políticas
parciais.  Uma política parcial é removida da pilha no momento em que
seu objetivo é atingido pelo agente, ou seja, o evento saliente
utilizado para criá-la é disparado.  Como uma política parcial $p$
pode invocar outras de modo a atingir seu objetivo, $p$ é removida da
pilha juntamente com as políticas parciais invocadas por ela.  A
estratégia de remoção testa as políticas parciais para remoção
partindo da base da pilha em direção ao topo, ou seja, prioriza a
remoção das políticas parciais mais anteriormente adicinadas.
...
\end{tabular}
  ...
  \label{fig:obj}
\end{wrapfigure}
se econtra em um reticulado $5\times5$ e deve descobrir como ativar um
\emph{alarme} interagindo com objetos aí presentes. O agente é
composto de um \emph{olho}, uma \emph{mão} e um \emph{alvo} e possui
sempre disponíveis as seguintes ações: apontar \emph{olho} para a
célula onde se encontra a \emph{mão} ou para a célula onde se encontra
o \emph{alvo}; mover a \emph{mão} ou o \emph{alvo} para a célula
atualmente apontada pelo \emph{olho}.
...
\beon.  O \emph{alarme} dispara quando ocorre \beon\ a luz está ligada
e a música desligada simultaneamente e é desativado quando o estado da
luz ou da música é modificado; o agente somente consegue diferenciar
os botões e agir sobre eles se a luz estiver ligada.
...
parciais aplicadas duranteo o aprendizado, removendo-as através de
...
aprendizado proposta frente a abordagens mais clássicas e
comportamento aleatório.  Os resultados obtidos sugerem a
...
trabalhos futuros, pretende-se a esteder o algoritmo para a solução de

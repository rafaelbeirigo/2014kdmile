...
%% Tradução para português da referência usada para algoritmos
...
\begin{algorithmic}[1]                    % enter the algorithmic environment
    \LOOP
    %% \STATE $s_t \leftarrow$ estado atual
    %% \STATE $a_t \leftarrow$ ação primitiva atual
    %% \STATE $o_t \leftarrow$ macro-ação atual
    %% \STATE $r^e_t \leftarrow$ recompensa extrínsica atual
    %% \STATE $r^i_t \leftarrow$ recompensa intrínsica atual
    %% \STATE \\
    %% \STATE Obter próximo estado $s_{t+1}$
    %% \STATE \\
    %% \STATE \\
    %% \COMMENT{Lida com a possível ocorrência de um evento saliente}
    ...
    %% \STATE \\
    %% \STATE \\
    %% TODO: parece que tem erro na linha abaixo (talvez o \tr...)
    %% \COMMENT{Atualização das \tr{funções valor estado-ação} para
    %%   as macro-ações}
    \FOR{cada macro-ação $o$ em $O$ tal que $s_t \in I^o$}
        ...
        \newcommand{\termoA}{\bo{\sd} $\times$ \tv}
        \newcommand{\termoB}{$($\ubo{\sd}$)\ \times$ \maos}

        \STATE $a \leftarrow$ \termoA
        \STATE $b \leftarrow$ \termoB

        \newcommand{\aalpha}{$\xleftarrow{\alpha}$}
        \STATE $Q^o(s_t,\ a_t)$ \aalpha $\ r^e_t\ +\ \gamma(a\ +\ b)$
        \FOR{cada macro-ação $o'$ em $O$ tal que $s_t \in I^{o'}$ e $o
          \neq o'$}
            \newcommand{\sumxes}[1]{$\sum_{x \in S}{$#1$}$}
            \STATE $c \leftarrow$ \bo{$x$}$\ \times\ $\tv
            \STATE $d \leftarrow$ $($\ubo{$x$}$)\ \times\ $ \maox
            \STATE $Q^o(s_t,\ o')$ \aalpha $R^{o'}(s_t)\ + $
            \sumxes{$P^{o'}(x|s_t)$}$[c\ +\ d]$ 

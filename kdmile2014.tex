...
são conjuntos finitos de estados e ações respectivamente, $R \colon S
\to \Re$ representa a recompensa \emph{extrínseca} $r^e$ recebida ao
alcançar um estado $s \in S$ e $P \colon S \times A \times S \to
[0,1]$ a probabilidade de alcançar um estado a partir de outro através
da execução de uma ação.  Uma política $\pi \colon S \to A$ associa
uma ação a ser executada em um estado e a função $V^{\pi} \colon S \to
\Re$ fornece o valor esperado de recompensas futuras ao, partindo de
$s \in S$, executar $\pi$.  A solução de um PDM é dado pela política
$\pi^*(s) = \arg\max_{\pi}{V^{\pi}(s), \forall s \in S}$, denominada
\emph{ótima}, obtida pelo Q-Learning \cite{watkins1989learning}
iterativamente por
%
...
%
onde $\alpha \in\ ]0,1]$ e $\gamma \in [0,1]$ correspondem à \emph{taxa
    de aprendizado} e \emph{fator de desconto futuro},
respectivamente.



%  através da atualização
% \emph{exploração}/\emph{aplicação}.  
% uma
% técnica de Aprendizado por Reforço. Nesse algoritmo, o a seleção da
% ações a serem executadas é feita pela estratégia $\epsilon$-gulosa,
% $\epsilon \in [0,1]$, que retorna ações aleatórias com probabilidade
% $\epsilon$ e gulosas com probabilidade $1 - \epsilon$, sendo a
% atualização 

% da função $Q: S\times A\rightarrow \Re$, que indica o valor de se e

% sendo feita atualização iterativa da função $Q: S \times
% A \rightarrow \mathcal{R}$ por
% %
% %
% sendo feita atualização iterativa da função $Q: S \times
% A \rightarrow \mathcal{R}$ por
% %
% \newcommand{\qt}{Q(s_t, a_t)}
% $\qt\ \leftarrow (1 - \alpha)\qt\ + \alpha (r_t + \gamma \max_a)$
% %
...
\rightarrow \OsetS$, associa ações primitivas ou políticas parciais a
...
original é realizada através da divisão deste em subproblemas
...
\begin{wrapfigure}{r}{0.5\textwidth}
  ...
  \end{minipage}
\end{wrapfigure}
Dessa forma, introduziu-se o parâmetro $\epS \in [0,1]$, que indica o
    ...
    \begin{center}
      \begin{figure}[H]
        ...
        \caption{Avaliação da estratégia de pilhas de políticas
            parciais para exploração interna com diferentes valores de
            \ep, \emph{versus} Q-Learning e comportamento aleatório}
        \label{fig:ep}
      \end{figure}
    \end{center}
  \end{minipage}
...
parcial, foram realizados 100 experimentos de \quinhentosmil\ passos
cada com a mesma parametrização anterior, mas com valores de $\epS \in
\{0.2, 0.4, 0.6, 0.8\}$.  O desempenho foi comparado com um agente
...
média cumulativa de ativações do \tmon\ por passo de experimento.
Podemos perceber um desempenho inicial desfavorável da técnica frente
ao Q-Learning.  Isso pode ser explicado pelo custo adicional da
técnica relacionado ao aprendizado das políticas parciais.
Entretanto, o desempenho cumulativo da técnica apresentou evolução
mais rápida e melhor desempenho final, sugerindo a vantagem do
aprendizado inicial das políticas parciais para posterior reuso.
Pode-se também perceber que o valor de \ep\ possui impacto
significativo no aprendizado, sendo o desempenho apresentado
diretamente proporcional ao valor de \ep.

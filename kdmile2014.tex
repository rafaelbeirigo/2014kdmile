...
recompensas futuras ao se executar a ação $a$ no estado $s$, sendo o
valor \emph{ótimo} dado por $Q^*(s) = \max_a Q(s, a)$ , que pode ser
obtida iterativamente pelo Q-Learning \cite{watkins1989learning} por
%
...
$\pi^*(s) = \arg\max_a Q^*(s, a), \forall s \in S$, uma vez obtida
$Q^*$.  No Q-Learning as ações são selecionadas pela estratégia
$\epsilon$-gulosa, que recomenda uma ação aleatórias e ótimas com
probabilidades $\epsilon \in [0,1]$ e $1 - \epsilon$, respectivamente.
Dessa forma, o agente aplica o conhecimento adquirido sobre o
problema, aprimorando-o através de exploração.
...
original é realizada através da divisão deste em subproblemas menores,
cuja solução é obtida através do aprendizado de políticas parciais.
